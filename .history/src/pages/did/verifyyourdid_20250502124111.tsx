...
const Mint: React.FC = () => {
  ...
  // Handle file upload
 // Kiểm tra phần code khởi tạo file, đảm bảo nó được gán như thế này:
const handleFileChange = (e) => {
  if (e.target.files && e.target.files[0]) {
    setFile(e.target.files[0]); // Đảm bảo file là một File object hợp lệ
...
// Upload image to Pinata using API route
// Upload image to Pinata using API route
// Upload image to Pinata using API route
   // Sửa hàm uploadToPinata
const uploadToPinata = async () => {
  if (!file) throw new Error("Không có file được chọn");

  try {
    // Kiểm tra xem file có phải là File object hợp lệ không
    ...
    if (!response.ok) {
      ...
      throw new Error(responseData?.error || "Pinata upload failed");
    }
    
    if (!responseData?.cid) {
      console.error("Missing CID in response", responseData);
      throw new Error("Server không trả về CID");
    }
    
    console.log("Upload successful:", responseData);
    
    const ipfsUrl = `ipfs://${responseData.cid}`;
    return { 
      ipfsUrl, 
      fileType: file.type 
    };
