...
type ChatContextType = {
    ...
    Message[] >; // Lưu tin nhắn theo địa chỉ ví
...
export const ChatProvider = ({children, walletAddress} : {
    children: ReactNode,
    walletAddress?: string
}) => {
    const [socket,
        setSocket] = useState < Socket | null > (null);
    const [isOpen,
        setIsOpen] = useState(false);
    const [messages,
            setMessages] = useState < Record < string,
        Message[] >> ({});
    const [recipientAddress,
        setRecipientAddress] = useState('');
    const [message,
        setMessage] = useState('');
    const [currentChat,
        setCurrentChat] = useState < string | null > (null);
    const [allContacts,
        setAllContacts] = useState < string[] > ([]);
    const [connectionStatus,
        setConnectionStatus] = useState < 'connected' | 'connecting' | 'disconnected' | 'error' > ('disconnected');
    const [socketInitialized,
        setSocketInitialized] = useState(false);
        ...
        if (typeof window !== 'undefined' && walletAddress) {
            try {
                ...
                if (savedContactsString) {
                    ...
                    // Nếu có liên hệ, tự động tải lịch sử chat cho mỗi liên hệ khi socket đã kết
                    // nối
    ...
    // Khởi tạo Socket.IO
    useEffect(() => {
        if (!walletAddress || socketInitialized) 
            return;
    ...
    useEffect(() => {
        ...
        if (socket.connected && walletAddress) {
            ...
            if (allContacts.length > 0) {
                socket.emit('sync_contacts', {walletAddress, contacts: allContacts});
            }
        ...
        socket.on('unread_messages', (unreadMessages : Message[]) => {
            ...
            const groupedMessages : Record < string,
                Message[] > = {};
    ...
    const sendMessage = () => {
        if (!socket || !message.trim() || !recipientAddress || !walletAddress) 
            return;
